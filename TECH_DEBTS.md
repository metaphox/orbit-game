...
| TD-3 | Visuals hardcoded (materials, shaders, colors, meshes inline) — blocks the "Themes" feature | `src/ui/flight_view.gd`, `map_view.gd`, shaders | **Paid** — the `RenderTheme` seam is proven swappable end-to-end, and **every flight-view surface colour now reads from it**: env/atmosphere/bodies/trajectory, plus the player craft's typed `ShipSkin`, target ring, corridor band, node ghost, all seven orbit marks, prograde/retrograde, and the posture marker (theme threaded into `ManeuverVisuals` + `ShipVisuals` as an optional param). The craft OBJ now supplies only geometry and stable material-slot names; a theme-selected `.tres` skin explicitly overrides every surface, ready for future texture-backed materials. **DA-2 pass:** the per-body procedural-ramp palette in `celestial_body.gdshader` (Earth cloud/grid, Moon dust/maria, Sun plasma, Mars terrain/ice) was hardcoded so `RenderTheme.body_colors` only affected generic bodies; those ramp stops are now `RenderTheme.body_ramps` shader uniforms wired in `BodyRenderer`, so a theme restyles a named body's whole surface (proven at the material-uniform level by `test_body_invariants`). **Intentional exceptions:** the chase-camera fill light + star-dust particle tint stay inline (lighting/particle detail, not themeable surfaces); the ship **engine-flame** shader (`ship_flame.gdshader`) keeps its amber→white-hot ramp inline (a single additive VFX, not a themeable body surface); the rendezvous station is a shared `.tscn` (`station_model`); and the minimap (`map_view`) deliberately draws the semantic `Palette` colours (own=green, target=cyan) rather than a swappable theme. **CR-6 pass:** the sun lens-flare tints moved into `RenderTheme` (`flare_*`), and the remaining stray literals (debug-grid, minimap ship-nose, map label-shadow, `TRANSPARENT`) moved into `Palette`; the one baked minimap colour in `hud_layout.tscn` is now transparent (the real fill is applied at runtime from `Palette.MAP_BG`). A seam lint (`tools/lint_ui_colors.sh`, wired into `tools/test.sh`) now rejects any raw `Color(<number>...)` in `src/ui` outside `palette.gd`/`render_theme.gd`; the two inline exceptions carry a `# lint-ok:` marker. **Coverage gap (post–UI-revamp):** the lint scans `.gd` only, not `.tscn`/`.tres`. New menu/HUD chrome scenes stay colour-clean by using `theme_type_variation`; the player-ship material resources are owned by `RenderTheme.ship_skin`, while two 3D **material** scenes still hold baked colours — `station_model.tscn` (a documented exception) and `map_view_layout.tscn`'s minimap-grid materials (undocumented; small, pre-existing). Not machine-enforced. |

...
| TD-2 | `flight_view.gd` is a ~1000-line god object rendering cameras, bodies, trajectory, markers, station, hologram, starfield, node visuals — all in one file | `src/ui/flight_view.gd` | **In progress** (Phase 4) — `BodyRenderer` extracted (993→813 lines). Remaining collaborators: CameraRig, TrajectoryRenderer, ManeuverVisuals, ShipVisuals. |

...
var _queue: Array = []
var _current: Phase = null   # the active Phase, or null between phases
...
func update(game: GameRoot) -> void:
...
func _burn(label: String, dir_fn: Callable, done_fn: Callable) -> Phase:
	var upd := func(_dir: FlightDirector, game: GameRoot) -> void:
		game.warp_index = 0
	...
	return Phase.new(label, upd, func(game: GameRoot) -> bool: return done_fn.call(game.ship))
...
func _burn_to_value(label: String, dir_fn: Callable, value_fn: Callable,
	...
	var done := func(game: GameRoot) -> bool: return absf(value_fn.call(game.ship) - target) <= tol
...
func _burn_toward(label: String, vt_fn: Callable) -> Phase:
	var upd := func(dir: FlightDirector, game: GameRoot) -> void:
		game.warp_index = 0
		...
		game.ship.throttle = 1.0
	var done := func(_game: GameRoot) -> bool: return _current != null and _current.state.get("settled", false)
	var enter := func(_dir: FlightDirector, _game: GameRoot) -> void:
...
func _coast_to_time(label: String, time_fn: Callable, observe := false) -> Phase:
	var enter := func(_dir: FlightDirector, game: GameRoot) -> void:
	...
	var upd := func(dir: FlightDirector, game: GameRoot) -> void:
		...
		game.warp_index = wi
	var done := func(game: GameRoot) -> bool: return game.sim_time >= _current.state["t"] - 1e-6
	return Phase.new(label, upd, done, enter)
...
func _coast_until(label: String, pred: Callable, warp_idx := -1) -> Phase:
	...
	return Phase.new(label, upd, func(game: GameRoot) -> bool: return pred.call(game.ship))
...
func _warp_for_remaining(game: GameRoot, remaining: float) -> int:
	if remaining <= 0.0:
		return 0
	var max_step := remaining / (2.0 * (1.0 / 60.0))
	var wi := 0
	for i: int in game.WARP_STEPS.size():
...
func _build_plan(game: GameRoot) -> Array:
	var obj: Objective = game.level.objective
	if obj is OrbitMatchObjective:
		return _plan_orbit_match(game, obj as OrbitMatchObjective)
	if obj is RendezvousObjective:
		return _plan_rendezvous(game, obj as RendezvousObjective)
	if obj is TransferCaptureObjective:
		if game.ship.body.parent == null:
			return _plan_transfer_capture(game, obj as TransferCaptureObjective)
		return _plan_interplanetary(game, obj as TransferCaptureObjective)
	if obj is EntryCorridorObjective:
		return _plan_return(game, obj as EntryCorridorObjective)
	if obj is AirlessLandingObjective:
		return _plan_landing(game, obj as AirlessLandingObjective)
	return []


func _plan_orbit_match(game: GameRoot, obj: OrbitMatchObjective) -> Array:
	...
	if target_inc > 1e-4:
		...
		phases.append(_coast_to_time("COAST TO APOAPSIS",
			func(g: GameRoot) -> float: return OrbitEvents.apoapsis_time(g.ship.current_elements(), g.sim_time)))
		phases.append(_burn("RECIRCULARIZE", func(s: ShipSim) -> DVec3: return s.v,
			func(s: ShipSim) -> bool: return s.v.length() >= sqrt(s.body.mu / s.r.length())))
	if target_r > r0 + 1.0:
		phases.append(_burn("RAISE APOAPSIS", func(s: ShipSim) -> DVec3: return s.v,
			func(s: ShipSim) -> bool: return s.current_elements().radius_apoapsis() >= target_r))
		phases.append(_coast_to_time("COAST TO APOAPSIS",
			func(g: GameRoot) -> float: return OrbitEvents.apoapsis_time(g.ship.current_elements(), g.sim_time)))
		...
		phases.append(_burn("CIRCULARIZE", func(s: ShipSim) -> DVec3: return s.v,
			func(s: ShipSim) -> bool: return s.v.length() >= sqrt(s.body.mu / s.r.length())))
	elif target_r < r0 - 1.0:
		phases.append(_burn("LOWER PERIAPSIS", func(s: ShipSim) -> DVec3: return s.v.neg(),
			func(s: ShipSim) -> bool: return s.current_elements().radius_periapsis() <= target_r))
		...
		phases.append(_burn("CIRCULARIZE", func(s: ShipSim) -> DVec3: return s.v.neg(),
			func(s: ShipSim) -> bool: return s.v.length() <= sqrt(s.body.mu / s.r.length())))
	return phases


func _plan_rendezvous(game: GameRoot, obj: RendezvousObjective) -> Array:
	...
	return [
		_coast_to_time("PHASING", func(_g: GameRoot) -> float: return fire_at),
		_burn("HOHMANN BURN", func(s: ShipSim) -> DVec3: return s.v,
			func(s: ShipSim) -> bool: return s.current_elements().radius_apoapsis() >= r_dest),
		_coast_to_time("COAST TO APOAPSIS",
			func(g: GameRoot) -> float: return OrbitEvents.apoapsis_time(g.ship.current_elements(), g.sim_time)),
		_burn_toward("MATCH STATION", func(s: ShipSim) -> DVec3: return so.state_at_time(s.last_time).v),
...
func _terminal_rendezvous(label: String, so: OrbitElements, max_dist: float, max_rel: float) -> Phase:
	var upd := func(_dir: FlightDirector, game: GameRoot) -> void:
		...
		if residual.length() > V_CUT:
			...
			s.throttle = 1.0
	var done := func(game: GameRoot) -> bool:
		var s: ShipSim = game.ship
		var st := so.state_at_time(s.last_time)
...
func _plan_transfer_capture(game: GameRoot, obj: TransferCaptureObjective) -> Array:
...
func _plan_interplanetary(game: GameRoot, obj: TransferCaptureObjective) -> Array:
	var target: BodyDef = obj.target
	...
	var lambert_leg := Phase.new("MARS INTERCEPT (LAMBERT)",
		func(_d: FlightDirector, _g: GameRoot) -> void: pass, func(_g: GameRoot) -> bool: return true)
	lambert_leg.on_enter = func(dir: FlightDirector, run_game: GameRoot) -> void:
		var s: ShipSim = run_game.ship
		var t_now: float = s.last_time
		var best_v1: DVec3 = null
		var best_dv := INF
		for frac: int in range(50, 160, 5):
	...
	return [
		_coast_to_time("DEPARTURE PHASING", func(_g: GameRoot) -> float: return fire_at, true),
		...
		_burn("EARTH DEPARTURE", func(s: ShipSim) -> DVec3: return s.v, func(s: ShipSim) -> bool:
			var el := s.current_elements()
			var vinf := 0.0 if el.is_elliptic() else sqrt(maxf(-s.body.mu / el.a, 0.0))
			return vinf >= v_inf_target),
		_coast_until("ESCAPE EARTH SOI", func(s: ShipSim) -> bool: return s.body == sun),
		lambert_leg,
	]


func _plan_return(game: GameRoot, obj: EntryCorridorObjective) -> Array:
	...
	if game.level.fail_radius > 0.0:
		...
		phases.append(_burn("LOWER RETURN APOAPSIS", func(s: ShipSim) -> DVec3: return s.v.neg(),
			func(s: ShipSim) -> bool: return s.current_elements().radius_apoapsis() <= cap))
	phases.append_array([
		_coast_to_time("COAST TO APOAPSIS",
			func(g: GameRoot) -> float: return OrbitEvents.apoapsis_time(g.ship.current_elements(), g.sim_time)),
		_burn_to_value("TRIM PERIAPSIS", func(s: ShipSim) -> DVec3: return s.v.neg(),
			func(s: ShipSim) -> float: return s.current_elements().radius_periapsis(), target_pe, tol * 0.5),
	])
	return phases


func _plan_landing(_game: GameRoot, obj: AirlessLandingObjective) -> Array:
	var target: BodyDef = obj.target
	var radius: float = target.radius
	return [
		_burn("DEORBIT", func(s: ShipSim) -> DVec3: return s.v.neg(),
			func(s: ShipSim) -> bool: return s.current_elements().radius_periapsis() <= radius * 0.9),
...
func _descent(label: String, target: BodyDef) -> Phase:
	...
	var done := func(game: GameRoot) -> bool: return game.ship.r.length() <= radius
	return Phase.new(label, upd, done)

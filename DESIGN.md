...
- **World (chase + orbit views):** Apollo/Skylab hardware language on the ship — a weathered NASA-punk material treatment with a warm matte hull, muted beige insulation, restrained amber markings, metallic frames/nozzles, dark solar cells, and subdued gold foil — against a procedural starfield sky (hashed voxel grid, no texture assets) and a whole-screen film grade (grain, vignette, warm tint) that reads as aged hardware footage, not a modern render. The close-up craft's materials are a named `ShipSkin` selected by `RenderTheme`: the OBJ supplies geometry and stable material-slot names, while the skin supplies explicit Godot material overrides. This keeps the default flat-PBR treatment replaceable by a future texture-backed skin without changing the ship scene or simulation. Planets get a cheap rim/fresnel glow for an atmosphere hint.
- **Station operators:** stations share that late-70s industrial universe without
  all belonging to the player's shipyard. The procedural station generator has
  four fictional operator brands—Japanese, Chinese, American, and German—which
  resolve common hull/structure/solar/accent/light roles to distinct, restrained
  colour-and-finish families and one original primitive hull mark. Brand selection
  is deterministic and cosmetic in its first phase; it cannot alter structural
  topology, connectivity, thermal sizing, or organisation scoring. Exact names,
  materials, references, and acceptance rules live in `docs/STATIONS.md` §8.
...
**Why the architecture makes this cheap.** The universe is a pure function of `sim_time` (bodies on rails), coasting is closed-form and time-symmetric (`OrbitElements.state_at_time`), and `ShipSim.serialize()`/`apply_serialized()` round-trip the dynamic state (it's the save system). A **persisted** save deliberately resumes COASTING (a mid-burn substep isn't meaningful across a save boundary); an in-session **live** snapshot (`apply_serialized(..., live = true)`) additionally restores throttle and flight state, so CANCEL returns to "now" unchanged and burns stay atomic (§14.1–14.2). Rewind is a small in-memory snapshot buffer over primitives that already exist and are tested. Powered flight (RK4 + mass loss) is path-dependent and not reversible, so rewind does **not** replay the discarded burn: `RewindBuffer` keeps only **discrete anchors** — launch, each pre-burn coast state, and SOI landmarks — and stepping restores the selected anchor exactly. The 0.5 s reverse-sweep between anchors is an **illustrative coast interpolation** of that anchor's own conic evaluated backward (everything renders from `sim_time`), not a frame-by-frame replay of the actual burned path; only the discrete anchors are historical truth. (A true ghost of the discarded trajectory is a listed fast-follow, not first cut.)
